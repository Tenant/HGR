...
AmplitudeScaleStepSize = round((AmplitudeScaleMax - AmplitudeScaleMin)/10);
...
colormap(flipud(jet)); cbr = colorbar;
DistanceScaleStepSize = round((DistanceScaleMax - DistanceScaleMin)/10);
...
ii=1;
while(isRunning == figCount && ii<=320)
...
%         if (status == 0); ADCTestLine = typecast(uint8(ADCTestLine),'uint16'); ADCTestLine = bitshift(ADCTestLine,-4); end
    ...
    coef_rad = (0.5*299792458/modulationFrequency)*1000 / (2*pi);
    PhasePi = Phase + pi; % +pi to compensate negative ATAN2 values
    PhasePiCoef = PhasePi * coef_rad;
    distData = PhasePiCoef+ DistOffset00;

    frames{ii}.ampData=ampData;
    frames{ii}.distData=distData;
    ii=ii+1;
